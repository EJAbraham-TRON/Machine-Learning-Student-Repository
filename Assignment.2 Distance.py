...
# Note: dropping duplicates reduces the dataset from 1,014 to 452 rows,
# and compromises the minimum row requirement for this assignment.
...
# StandardScaler for distance-based classifiers
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)
X_test_scaled  = scaler.transform(X_test)


# DEFINE DISTANCE-BASED CLASSIFIERS
...
print("Evaluation Metrics — Distance-Based Classifiers:")
...
for idx, (name, (clf, y_pred)) in enumerate(trained_models.items()):
...
plt.suptitle('Confusion Matrices — Distance-Based Classifiers', fontweight='bold', fontsize=13)
plt.tight_layout()
plt.show()
...
width = 0.18
colors_bar = ['Blue', 'Turquoise', 'green', 'Orange']
...
plt.ylim(0, 1.1)
plt.ylabel('Score')
plt.title('Classifier Performance Comparison — Distance-Based Methods')
plt.legend()
plt.tight_layout()
plt.show()
...
best_clf, best_y_pred = trained_models[best_name]
print(f"\nBest Distance-Based Classifier: {best_name}")
